...
const makeNodesWithVal = function(root) {
    
    root.val = root.item
    if (root.left) root.left = makeNodesWithVal(root.left)
    if (root.right) root.right = makeNodesWithVal(root.right)
    return root
}
...
var sortedListToBST = function(head) {
    ...
    let temp = head
    while (temp){
        avl.insertNode(temp.val)
        temp = temp.next
    }
    if (avl && head){
        avl = makeNodesWithVal(avl.getRoot())
        return avl
    }
    else return head
};